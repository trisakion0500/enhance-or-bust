...
### Coupon (11000번대) — coupon_platform 연동

| 코드 | HTTP | 메시지 |
|---|---|---|
| 11000 | 400 | 요청 값이 올바르지 않습니다. |
| 11001 | 404 | 존재하지 않는 쿠폰 코드입니다. |
| 11002 | 409 | 이미 사용된 쿠폰 코드입니다. |
| 11003 | 409 | 지금은 사용할 수 없는 쿠폰입니다. |
| 11004 | 409 | 쿠폰 사용 한도를 초과했습니다. |
| 11999 | 500 | 일시적인 서버 오류입니다. 잠시 후 다시 시도해주세요. |

### Attendance (12000번대)

| 코드 | HTTP | 메시지 |
|---|---|---|
| 12000 | 400 | 요청 값이 올바르지 않습니다. |
| 12001 | 404 | 존재하지 않는 출석부입니다. |
| 12002 | 409 | 이미 지급된 날짜입니다. |
| 12003 | 409 | 골드가 부족합니다. |
| 12004 | 409 | 캐치업 구매 가능 횟수를 초과했습니다. |
| 12005 | 409 | 이미 종료된 출석부입니다. |
| 12006 | 409 | 기존 출석부 기간과 겹칩니다. (gm_platform 저장 API 전용 — 현재 미사용) |
| 12007 | 409 | 이미 등록된 출석부입니다. (gm_platform 저장 API 전용 — 현재 미사용) |
| 12008 | 409 | 이미 시작된 출석부는 수정할 수 없습니다. (gm_platform 저장 API 전용 — 현재 미사용) |
| 12009 | 400 | 보상/캐치업 행 구성이 설정값과 일치하지 않습니다. (gm_platform 저장 API 전용 — 현재 미사용) |
| 12999 | 500 | 일시적인 서버 오류입니다. 잠시 후 다시 시도해주세요. |
