...
적재해둔 `masterDataCache`를 그대로 읽어 반환한다(DB 재조회 없음). **1차는 조회만
지원하고 수정/삭제는 아직 없다** — 밸런스 데이터라 잘못 저장되면 게임 전체에 영향을 줄
수 있어 별도 검증 설계 후 추가 예정(출석 정의 3종도 마찬가지 — 현재 `npm run seed`로만
적재된다, `04_DATA_MODEL.md` 참고). **모두 X-API-Key 필요.**
