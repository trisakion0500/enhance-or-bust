...
/** `batch_runs` 컬렉션 문서 — `{jobName}:{period}`를 `_id`로 삼아 유니크 삽입으로 실행권을 선점한다. */
...
/**
 * 배치 잡의 인스턴스(레플리카) 간 중복 실행을 막는 멱등 마커. `batch_runs` 컬렉션에
...
export async function tryClaimBatchRun(db: Db, jobName: string, period: string): Promise<boolean> {
  try {
    await db.collection<BatchRunDocument>("batch_runs").insertOne({ _id: `${jobName}:${period}`, claimedAt: new Date() });
    return true;
  } catch (err) {
