...
export function renderInventory(state) {
  ...
  const rows = cards
    .map(c => `<tr><td>${escapeHtml(c.templateId)}</td><td>${escapeHtml(c.grade)}</td><td>${c.baseAttack}</td><td>${c.baseHp}</td><td>${escapeHtml(c.element)}</td><td>${c.level}</td><td>${c.exp}</td><td>+${c.enhancementLevel}</td></tr>`)
    .join("");
