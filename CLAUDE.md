...
- 부트스트랩(Mongo/Redis 연결, log4js 로깅, 에러 핸들링 스켈레톤, Express 서버 골격),
  ...
  없이 저장 생략. 우편함 미구현이라 보상은 우편 경유 없이 직접 지급 — Mailbox 구현 시
  전환 예정)까지 구현됨
- 미구현: Mailbox 도메인 서비스 로직(우편 발송/수령), Redis 분산락(현재 강화/합성/전투
  API는 재시도로만 동시 요청을 흡수하며, 재시도 폭주 방지용 락은 아직 없음), 인벤토리
