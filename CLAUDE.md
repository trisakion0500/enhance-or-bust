...
- 부트스트랩(Mongo/Redis 연결, log4js 로깅, 에러 핸들링 스켈레톤, Express 서버 골격),
  ...
  분리)까지 구현됨
- 미구현: 만료 우편 정리 배치 잡(크론/배치 인프라 자체가 아직 없음; 수령한 우편도
  데이터 가치가 있어 TTL 인덱스로 자동 삭제하지 않기로 함 — 정리가 필요해지면 미수령+
  만료분만 선별 삭제하거나 아카이브 컬렉션으로 이동하는 방식을 검토), Redis 분산락(현재
  강화/합성/전투/우편 API는 재시도 또는 트랜잭션으로만 동시 요청을 흡수하며, 재시도
  폭주 방지용 락은 아직 없음), 인벤토리 슬롯 상한, 스테이지 클리어 보상의 확률적 카드
