...
- 부트스트랩(Mongo/Redis 연결, log4js 로깅, 에러 핸들링 스켈레톤, Express 서버 골격),
  ...
  대상 카드 강화 단계 +1. 둘 다 강화 API와 동일한 낙관적 락 재조회·재시도 패턴), 전투/
  스테이지 API(`POST /battle-stage/:stageId/clear` — 출전 스쿼드 vs 몬스터 라운드제
  시뮬레이션, 승리 시 골드/EXP(출전 카드에만)/확률적 강화석을 Player 애그리게잇에 직접
  지급(파밍 재도전 시 `farmRewardRate`만큼 축소). `clearedStage`는 최초 클리어
  (`clearedStage+1`)일 때만 갱신되고, 파밍 승리는 갱신하지 않는다. 패배 시 상태 변경
  없이 저장 생략), Mailbox 도메인(`sendMail()` — (sourceType, sourceId) 유니크 인덱스로
  멱등 발송, HTTP API 없이 다른 Use-case가 내부 호출; `POST /mailbox/:mailId/claim` —
  mailbox+players 두 컬렉션을 세션 기반 멀티도큐먼트 트랜잭션으로 묶어 우편 상태 변경과
  Economy/Inventory 첨부물 지급을 원자적으로 처리; `GET /mailbox` — 만료되지 않은 목록
  조회)까지 구현됨. 다만 스테이지 클리어 보상은 아직 Mailbox 경유로 전환하지 않고
  기존처럼 Player 애그리게잇에 직접 지급 — 기존 API 응답 계약/E2E 테스트를 바꾸는
  별개 변경이라 후속 작업으로 남겨둠
- 미구현: 스테이지 클리어 보상의 Mailbox 경유 전환, 만료 우편 정리 배치 잡(크론/배치
  인프라 자체가 아직 없음), Redis 분산락(현재 강화/합성/전투/우편 API는 재시도 또는
  트랜잭션으로만 동시 요청을 흡수하며, 재시도 폭주 방지용 락은 아직 없음), 인벤토리
  슬롯 상한, 스테이지 클리어 보상의 확률적 카드 드랍(GAME_DESIGN.md 6절에서 이번 스코프
  제외로 명시)
